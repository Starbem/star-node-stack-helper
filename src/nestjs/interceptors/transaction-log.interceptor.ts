...
export interface TransactionLogInterceptorOptions {
  microservice: string
  operation: string
...
export class TransactionLogInterceptor implements NestInterceptor {
  private readonly logger = new Logger(TransactionLogInterceptor.name)

  constructor(private readonly options: TransactionLogInterceptorOptions) {}
    ...
    return next.handle().pipe(
      tap((data) => {
        ...
        const transactionData: LogTransaction = {
          name: this.options.operation,
          microservice: this.options.microservice,
          transactionId,
  ...
  private buildTransactionContext(
    ...
    if (this.options.customContext) {
      const customContext = this.options.customContext(executionContext)
